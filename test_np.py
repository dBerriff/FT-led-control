...
async def np_arc_weld(nps_, pixel_, rgb_gamma_):
    ...
    while True:
        for _ in range(randrange(100, 200)):
            ...
            nps_[pixel_] = get_rgb_l_g_c(arc_rgb, level, rgb_gamma_)
            nps_.write()
            await asyncio.sleep_ms(20)
        for level in range(160, -1, -1):
            nps_[pixel_] = get_rgb_l_g_c(glow_rgb, level, rgb_gamma_)
            nps_.write()
            await asyncio.sleep_ms(20)
        await asyncio.sleep_ms(randrange(1_000, 5_000))

async def np_twinkler(nps_, pixel_, rgb_gamma_):
...
async def main():
    ...
    rgb_gamma = get_rgb_gamma(gamma)  # conversion tuple
    level = 127  # 0 - 255
    c_rgb = colours['orange']
    rgb = get_rgb_l_g_c(c_rgb, level, rgb_gamma)
    
    task_1 = asyncio.create_task(np_arc_weld(nps, 0, rgb_gamma))
    ...
    rgb_set = tuple([rgb_fns.get_rgb_l_g_c(c, level, rgb_gamma) for c in rgb_set])
